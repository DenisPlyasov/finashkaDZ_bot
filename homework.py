...
async def send_homework_for_date(
    ...
    if target_chat_id is None and update and getattr(update, "effective_chat", None):
        target_chat_id = update.effective_chat.id

    # Фолбэк: если вызывается из JobQueue, попробуем взять chat_id из job
    ...
    if target_chat_id is None:
        logger.warning("send_homework_for_date: chat_id неопределён (group=%s, date=%s)", group, date_str)
        return
